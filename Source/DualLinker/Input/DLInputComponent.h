 ...
 */
UCLASS()
class DUALLINKER_API UDLInputComponent : public UEnhancedInputComponent
{
	GENERATED_BODY()
public:
	UDLInputComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
template <class UserClass, typename FuncType>
void UDLInputComponent::BindNativeAction(const UDLInputConfig* InputConfig, const FGameplayTag& InputTag, ETriggerEvent TriggerEvent, UserClass* Object, FuncType Func, bool bLogIfNotFound)
{
	check(InputConfig);
